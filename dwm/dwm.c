...
updateoverlay(Monitor *m)
{
    ...
    if(m == selmon)
        color = scheme[SchemeSel][ColBg].pixel;
    else
        color = 0x00000000;
    XSetWindowBorder(dpy, m->overlay, color);
}
...
createoverlay(Monitor *m)
{
    ...
    wa.border_pixel = 0;
    m->overlay = XCreateWindow(dpy, root, 0, 0, 1, 1, borderpx, vinfo.depth, InputOutput, vinfo.visual, CWOverrideRedirect|CWColormap|CWBackPixel|CWBorderPixel, &wa);
...
{
    Monitor *m;
    if(i >= LENGTH(panels) || panel == panels[i])
        return;
    if(panel)
